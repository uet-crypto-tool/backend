...
class CurveDomainParamter(Enum):
    ...
    @classmethod
    def create(cls, curve_name) -> Curve:
        try:
            curve_params = CurveDomainParamter[curve_name].value
        except KeyError:
            raise ValueError(f"Unknown elliptic curve name: {curve_name}")

        try:
            sub_group = SubGroup(
                curve_params["p"], curve_params["g"], curve_params["n"], curve_params["h"])
            curve = Curve(curve_params["a"],
                          curve_params["b"], sub_group, curve_name)
        except KeyError:
            raise RuntimeError("Missing parameters for curve %s" % curve_name)
        return curve
